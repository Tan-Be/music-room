...
import { isSupabaseConfigured, profilesApi, supabase } from "@/lib/supabase";
...
export default function ProfilePage() {
	...
	// biome-ignore lint/correctness/useExhaustiveDependencies: loadProfile is intentionally recreated from the current authenticated user.
	useEffect(() => {
			...
			>
						...
						{editingUsername ? (
							...
							>
								<h2 style={{ color: "#e2e8f0", margin: 0, fontSize: "1.8rem" }}>
									{profile?.username || getUserDisplayName(user)}
								</h2>
								{profile && (
									<button
										type="button"
				...
				>
					{[
						...
						<button
							...
							key={tab.key}
							onClick={() =>
								setActiveTab(tab.key as "history" | "favorites" | "settings")
							}
							style={{
				...
				{activeTab === "history" && (
					<div>
						...
						{history.length === 0 ? (
							...
							>
								<p style={{ color: "#a1a1aa" }}>
									Вы ещё не слушали треки в комнатах
								</p>
								<a
									href="/rooms"
									style={{
				...
				{activeTab === "favorites" && (
					<div>
						...
						{favoriteRooms.length === 0 ? (
							...
							>
								<p style={{ color: "#a1a1aa" }}>
									У вас пока нет избранных комнат
								</p>
								<p
									style={{
										color: "#6b7280",
				...
				{activeTab === "settings" && (
					<div>
						...
						>
							<h4 style={{ color: "#e2e8f0", margin: "0 0 1rem 0" }}>
								Информация профиля
							</h4>
