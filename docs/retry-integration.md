# Интеграция Retry логики для Supabase

## ✅ Статус: Реализовано

Retry логика полностью реализована и интегрирована в проект для повышения надежности работы с Supabase.

## 📦 Реализованные компоненты

### 1. Основная библиотека (`src/lib/retry.ts`)

Три основные функции для работы с retry:

#### `retryWithBackoff<T>`

Базовая функция с exponential backoff и jitter.

**Параметры:**

- `maxRetries` - максимальное количество попыток (по умолчанию: 3)
- `baseDelay` - базовая задержка в мс (по умолчанию: 1000)
- `maxDelay` - максимальная задержка в мс (по умолчанию: 10000)
- `exponentialBackoff` - использовать экспоненциальную задержку (по умолчанию: true)
- `shouldRetry` - кастомная функция для определения, нужен ли retry
- `onRetry` - callback при каждой попытке

**Пример:**

```typescript
const data = await retryWithBackoff(async () => {
  const { data, error } = await supabase.from('users').select('*')
  if (error) throw error
  return data
})
```

#### `retrySupabaseQuery<T>`

Обёртка для SELECT запросов с автоматическими toast уведомлениями.

**Особенности:**

- Показывает toast при первой попытке retry
- Возвращает `null` при неудаче вместо выброса ошибки
- Логирует все ошибки

**Пример:**

```typescript
const room = await retrySupabaseQuery(async () => {
  const { data, error } = await supabase
    .from('rooms')
    .select('*')
    .eq('id', roomId)
    .single()
  if (error) throw error
  return data
})
```

#### `retryMutation`

Обёртка для INSERT/UPDATE/DELETE с меньшим количеством попыток.

**Особенности:**

- Только 2 retry попытки (вместо 3)
- Возвращает `boolean` вместо данных
- Безопасна для мутаций

**Пример:**

```typescript
const success = await retryMutation(async () => {
  const { error } = await supabase.from('tracks').insert([track])
  if (error) throw error
})
```

### 2. Умная логика shouldRetry

Автоматически определяет, когда нужен retry:

**Retry выполняется для:**

- ✅ Сетевых ошибок (network, timeout, fetch)
- ✅ 5xx ошибок сервера
- ✅ Временных проблем с подключением

**Retry НЕ выполняется для:**

- ❌ PGRST116 (Not found)
- ❌ 23505 (Unique violation)
- ❌ 23503 (Foreign key violation)
- ❌ 42501 (Insufficient privilege)
- ❌ auth/invalid-credentials
- ❌ auth/user-not-found

### 3. Exponential Backoff с Jitter

Задержки между попытками:

- Попытка 1: ~1000ms (1s + jitter)
- Попытка 2: ~2000ms (2s + jitter)
- Попытка 3: ~4000ms (4s + jitter)

Jitter (случайная вариация 30%) предотвращает "thundering herd" проблему.

## 🔌 Интеграция в проект

### Файлы с интегрированной retry логикой:

#### 1. `src/lib/auth.ts`

- ✅ `getUserProfile()` - использует `retrySupabaseQuery`
- ✅ `updateUserProfile()` - использует `retryMutation`

#### 2. `src/lib/chat-realtime.ts`

- ✅ `loadRecentMessages()` - использует `retrySupabaseQuery`
- ✅ `getMessageWithUserInfo()` - использует `retrySupabaseQuery`
- ✅ `sendMessage()` - использует `retryMutation`

## 📊 Тестирование

### Покрытие тестами: 93.33%

Все 13 тестов успешно проходят:

```bash
npm test -- src/lib/retry.test.ts
```

**Тесты проверяют:**

- ✅ Успешное выполнение с первой попытки
- ✅ Retry при сетевых ошибках
- ✅ Отсутствие retry для PGRST116
- ✅ Максимальное количество попыток
- ✅ Exponential backoff
- ✅ Callback onRetry
- ✅ Кастомная функция shouldRetry
- ✅ Toast уведомления
- ✅ Возврат null при неудаче
- ✅ Меньше попыток для мутаций

## 📚 Примеры использования

Файл `src/lib/retry-example.ts` содержит 10 практических примеров:

1. Базовое использование retryWithBackoff
2. Использование retrySupabaseQuery (рекомендуется)
3. Использование retryMutation для вставки
4. Кастомные настройки retry
5. Кастомная логика shouldRetry
6. Обновление с retry
7. Удаление с retry
8. Множественные операции с retry
9. Retry с прогрессом
10. Комбинация с другими утилитами (голосование)

## 🎯 Преимущества

1. **Надежность** - автоматическое восстановление после временных сбоев
2. **UX** - пользователи видят уведомления о повторных попытках
3. **Производительность** - exponential backoff снижает нагрузку на сервер
4. **Безопасность** - не retry для ошибок валидации и прав доступа
5. **Гибкость** - легко настраивается под конкретные случаи

## 🔧 Рекомендации по использованию

### Для SELECT запросов:

```typescript
const data = await retrySupabaseQuery(async () => {
  const { data, error } = await supabase.from('table').select('*')
  if (error) throw error
  return data
})
```

### Для INSERT/UPDATE/DELETE:

```typescript
const success = await retryMutation(async () => {
  const { error } = await supabase.from('table').insert([item])
  if (error) throw error
})
```

### Для кастомной логики:

```typescript
const data = await retryWithBackoff(
  async () => {
    // ваш код
  },
  {
    maxRetries: 5,
    baseDelay: 500,
    shouldRetry: error => {
      // ваша логика
    },
  }
)
```

## 📈 Метрики

- **Покрытие кода:** 93.33%
- **Количество тестов:** 13
- **Интегрированных файлов:** 2 (auth.ts, chat-realtime.ts)
- **Примеров использования:** 10

## 🚀 Следующие шаги

Можно дополнительно интегрировать retry логику в:

- Компоненты с прямыми Supabase запросами
- API routes
- Server actions
- Realtime subscriptions (с осторожностью)

---

**Дата реализации:** 27 ноября 2025
**Статус:** ✅ Полностью реализовано и протестировано
